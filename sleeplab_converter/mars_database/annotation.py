...
def time_from_startToSeconds(rtf_df, start_date_str, start_dt):
    for n in range(0, len(rtf_df)):
        # if rtf_df.loc[n, "Time_from_start"] is not None:
        time = dt.datetime.strptime(
        ...
        # else:
        #    if dt.datetime.strptime(start_date_str + '-' + rtf_df.iloc[n]['Start_time'].strip(), "%d/%m/%Y-%Hh%Mm%Ss")>=start_dt:
        #        rtf_df.loc[n, "Time_from_start"] = (start_dt - dt.datetime.strptime(start_date_str + '-' + rtf_df.iloc[n]['Start_time'].strip(), "%d/%m/%Y-%Hh%Mm%Ss")).seconds()
        #    else:
        #        rtf_df.loc[n, "Start_time"] = (start_dt-dt.datetime.strptime(start_date_str + '-' + rtf_df.iloc[n]['Start_time'].strip(), "%d/%m/%Y-%Hh%Mm%Ss") + timedelta(days=1)).seconds()
    return rtf_df
...
def annotationDeltamed(path, patient, edf_name):
    ...
    Start_timeToStart_datetime(dataTXT_df, start_date_str, start_datetime_dt)
    ...
    sleep_df = dataTXT_df.loc[dataTXT_df["Event_label"].isin(stages)].copy()
    ...
    # same // for info about uncontinuity during block change
    block_ends = sleep_df.loc[sleep_df["Event_label"] == "//"]
    ...
    time_from_startToSeconds(dataRTF_df, start_date_str, start_datetime_dt)
