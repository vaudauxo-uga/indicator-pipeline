    ...
    with open(edf_filepath, mode='rb', buffering=0) as f:
        ...
        for i in trange(len(target_signals), desc='Total'):
            ...
            for j in trange(header['records'], desc='Signal '+repr(i+1)):
                signals[i][j*target_samples[i]:(j+1)*target_samples[i]] = [int.from_bytes(f.read(2), byteorder='little', signed=True) * scalefac[i] + dc[i] for _ in range(target_samples[i])]
...
def read_edf_export_mne(edf_path: str,
                    digital: bool = False,
                    ch_names: Optional[list[str]] = None,
                    annotations: bool = False,
                    dtype: np.dtype = np.float32
    ...
    for i in ch_idx:
        if header['label'][i] == 'EDF Annotations' and annotations:
            ...
            s_func = partial(
                read_signal_from_path_mne, edf_path=edf_path, ch_name=header['label'][i], dtype=dtype)
            s_load_funcs.append(s_func)
