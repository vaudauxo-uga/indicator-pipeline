...
from sleeplab_converters.mars_microsleep_mslt_mwt import annotation
...
def parse_samplearrays(s_load_funcs, sig_headers, header):
    ...
    if type(header['startdate']) is dt:
        ...
        start_ts = dt.strptime(header['startdate'] +'-'+ header['starttime'], "%d.%m.%y-%H.%M.%S")
...
def parse_annotations(header, edf_path, edf_name) -> tuple[
    ...
    if type(header['startdate']) is dt:
        ...
        st_rec = dt.strptime(header['startdate'] +'-'+ header['starttime'], "%d.%m.%y-%H.%M.%S")
    ...
    if annot_df is not None:
        if st_rec != annot_df.iloc[0]['Start_time']:
            ...
            for n in range (0,len(annot_df)): # Update event lag from start of recording
                dif = annot_df.iloc[n]['Start_time']-st_rec #compare here to the start time of the recording
                annot_df.loc[n,'Time_from_start'] = dif.seconds

        for index, row in annot_df.iterrows():

            #push all events with original labels into event list
            events.append(models.Annotation[str](name = row['Event_label'], start_ts= row['Start_time'], start_sec=row['Time_from_start'], duration=row['Duration']))
        ...
        return events, AASMSleepStages, AASMevents, analysis_start, analysis_end, lights_off, lights_on, REC_TYPE
    else:
        return events, AASMSleepStages, AASMevents, analysis_start, analysis_end, lights_off, lights_on, REC_TYPE
...
def parse_edf(_edf_path: str) -> tuple[dt, ]:
    ...
    try:
        sig_load_funcs, sig_headers, header = edf.read_edf_export(Path(_edf_path), annotations=False)
    except:
        sig_load_funcs, sig_headers, header = edf.read_edf_export_mne(_edf_path, annotations=False)
...
def read_series(
    ...
    for edf_path in src_dir_series.iterdir():
        ...
        if len(edf_list)>1: # Check if there are more than one edf file (also then multiple annotation files are expected)
        ...
        for edfs in edf_list: #loop through the edf files
            ...
            if series_name == 'MSLT':
                if edfs.split('-')[0][-3:] == 'T13': #edf needs to be MSLT recording (MSLT = T12, MWT = T13)
                    continue
            elif series_name == 'MWT':
                if edfs.split('-')[0][-3:] == 'T12': #edf needs to be MWT recording
                    continue

            try:# Read signals from edf files
                start_ts, sample_arrays, header = parse_edf(edfs)
            except Exception as e:
                logger.warning(f'Skipping subject {edf_path.stem} and file {edfs} due to error in EDF parsing:')
                logger.warning(e)
            ...
            try: # Read annotations that correspond to edf filename (will fail if files are not correctly named or don't follow the normal structure)
                events, AASMSleepStages, AASMevents, analysis_start, analysis_end, lights_off, lights_on, rec_type = parse_annotations(header, edf_path, edf_name = edfs.split('/')[-1].split('.')[0])
                if not events:
                    events, AASMSleepStages, AASMevents, analysis_start, analysis_end, lights_off, lights_on, rec_type = parse_annotations(header, edf_path, edf_name = edfs.split('/')[-1].split('.')[0].split()[0])
                    if not events:
                        logger.warning(f'Cannot find annotations for subject {edf_path.stem}')
            except Exception as e:
                logger.warning(f'Skipping subject {edf_path.stem} due to error in annotation parsing:')
                logger.warning(e)
            ...
            if not events:
                ...
                annotations = {
                'original_annotations': models.Annotations(annotations = events, scorer='original'), 
                'manual_hypnogram': models.Hypnogram(annotations = AASMSleepStages, scorer='manual'),
                'manual_aasmevents': models.AASMEvents(annotations = AASMevents, scorer='manual')
                }

            if MULTI_EDF:
                metadata = models.SubjectMetadata(
                    subject_id = edf_path.stem + '_V' + edfs.split('\\')[-1].split('.')[0].split()[0].split('V')[-1],
            ...
            subjects[metadata.subject_id] = models.Subject(metadata = metadata,
...
def convert_dataset(
    ...
    for series_name in series:
        ...
        _series, _error_counts = read_series(
        src_dir, series_name)
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--ds_name', type=str, default='MARS_MICROSLEEP')
