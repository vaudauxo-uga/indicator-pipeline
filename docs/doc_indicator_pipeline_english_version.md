...
- `logs/` – Location of generated log files.
- `src/`
    - `indicator_pipeline/`
...
- The full execution follows three rules:
...
📝 **Note**: the default Snakefile paths point to the **user’s Desktop** (`~/Desktop`). If the project is executed from a different location, you must **adapt the paths defined in the `Snakefile` (variables `SLF_OUTPUT`, `LOGS_DIR`, etc.)** accordingly.
