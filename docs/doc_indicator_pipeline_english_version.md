...
- **Sleeplab conversion**: conversion of polysomnography files to the *sleeplab* format (via the `sleeplab_converter` module). The converted files are saved:
    - on the **storage server**, in the corresponding patient folder, under the name `slf_PAxxxx_Vx_FExxxx`;
...
- `ParametersValues_<calculation_date_and_time>`
    ...
    Contains a single Excel file `ParameterValues` that consolidates all the calculated parameter values from the *SLF* files (one row per PAxxxx_Vx_FExxxx file).
...
- **Class `SLFConversion`**
    ...
    **Methods**
    ...
    - `check_patient_recordings(remote_patient_path: PurePosixPath): Tuple[bool, List[str], bool]`
    
        Checks whether all T1 recordings for a patient already have an associated _slf_ folder. Returns:
            - `all_psg_converted: bool` => if all T1 recordings have an associated _slf_ folder
            - `missing_visits: List[Tuple[str, str]]` => list of recordings (e.g., ("V1", "FE0001")) without _slf_
            - `has_valid_psg: bool` => if the patient folder has at least one valid T1 recording to convert
...
- `df_to_json_payloads(df: pd.DataFrame): List[Dict[str, Any]]`
    ...
    - Extracts patient id, visit number, and recording number
...
- `excel_to_json(abosa_version: str): None`
...
- `utils.py` – Common utility functions used across multiple modules
    - `parse_recording_number(filename: str): str`
      
        Extracts the recording number _FExxxx_ from a string (psg or slf filename).

    - `parse_patient_visit_recording(filename: str) -> Tuple[str, str, str]`
        
        Extracts patient id, visit number and recording number from a string (filename containing patterns like *PxxxxVx* or *Pxxxx_Vx* and *FExxxx*). Returns a tuple with the patient ID, visit number and recording number as strings.
        
    - `extract_subject_id_from_filename(edf_file: Path) -> str`
        
        Extracts the patient ID, visit number and recording number from an EDF file path. Returns a string in the format `"PAxxxx_Vx_FExxxx"` (or `"PAxxxx"` if no visit and/or recording number are present).
        
    - `extract_recording_values(file_list: List[str]): List[Tuple[str, str]]`
    
        Extracts (visit, recording) tuples from PSG T1 filenames. Returns tuples in a list such as [('V1', 'FE0001'), ('V1', 'FE0002'), ('V2', 'FE0001')]
